    ...
    def _create_debug_panel(self) -> Panel:
        ...
        if not self.debug_logs:
            ...
            for log in list(self.debug_logs)[-20:]:
                ...
                elif event == 'search_files':
                    lines.append(f"[dim]{ts}[/dim] [blue]🔎 {agent}[/blue] Searching: {msg}")
                elif event == 'delete_file':
                    lines.append(f"[dim]{ts}[/dim] [red]🗑️ {agent}[/red] Deleting: {msg}")
...
Environment Variables:
    ...
    parser.add_argument('--device', type=str, choices=['auto', 'cuda', 'mps', 'cpu'], default=None)
    parser.add_argument('-p', '--parallel', type=int, default=None, help='Max parallel agents')
    ...
    parser.add_argument('--max-retries', type=int, default=None, help='Maximum retries for failed verifications')
    ...
    # Normalize invalid log levels early (prevents validation failures)
    try:
        LogLevel(config.logging.level.upper())
    except ValueError:
        invalid_level = config.logging.level
        config.logging.level = "INFO"
        console.print(f"[yellow]Invalid COUNSEL_LOG_LEVEL '{invalid_level}', using INFO.[/yellow]")
    ...
    if args.workspace:
        ...
        os.makedirs(config.shell.working_directory, exist_ok=True)
    elif config.shell.working_directory is None:
        cwd = os.getcwd()
        if os.path.basename(cwd) == "projects":
            default_workspace = cwd
        else:
            default_workspace = os.path.join(cwd, "projects")
        os.makedirs(default_workspace, exist_ok=True)
        config.shell.working_directory = default_workspace
    
    # Configure logging based on settings
    try:
        log_level = LogLevel.DEBUG if config.debug else LogLevel(config.logging.level.upper())
    except ValueError:
        invalid_level = config.logging.level
